...
1. 关于 BGR 顺序
   OpenCV 默认用 BGR 而不是 RGB，这是历史遗留问题（早年 Intel 开发的时候
   BGR 是主流）。刚学的时候很容易踩这个坑，比如用 Matplotlib 显示 OpenCV
   读的图，颜色就不对了。解决方法是用 cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
   转换一下。

2. 关于图像坐标
   OpenCV 里图像的坐标是 (行, 列)，也就是 (y, x)，跟数学里的习惯反过来。
   img[100, 200] 表示的是第 100 行、第 200 列的像素，也就是 y=100, x=200。
   一开始经常搞混，写多了就习惯了。

3. 关于像素访问效率
   直接用 img[y, x] 访问单个像素在 Python 里效率很低，如果要遍历所有像素，
   尽量用 NumPy 的向量化操作，或者用 OpenCV 自带的函数，速度差好几个数量级。

4. ROI 的浅拷贝问题
   用 img[y1:y2, x1:x2] 截取 ROI 时，返回的是原图像的视图（浅拷贝），
   修改 ROI 会影响原图。如果不想影响原图，记得用 .copy() 深拷贝。
============================================
"""
