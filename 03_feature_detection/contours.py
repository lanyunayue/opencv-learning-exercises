...
1. 轮廓检测的输入是二值图
   findContours 的输入通常是边缘检测（Canny）或阈值处理后的二值图像。
   如果直接用灰度图，效果会很差。先做 Canny 或 threshold 是常规操作。

2. 轮廓检索模式的选择
   - RETR_EXTERNAL：只检测最外层轮廓，最常用，速度也最快
   - RETR_LIST：检测所有轮廓，但不建立层级关系
   - RETR_TREE：检测所有轮廓，建立完整的层级树（嵌套关系）
   一般用 RETR_EXTERNAL 就够了，如果需要处理有孔洞的物体，再用其他的。

3. 轮廓近似 approxPolyDP
   这个函数超实用！可以把复杂的轮廓近似成简单的多边形。
   epsilon 参数控制近似精度，一般设为周长的 1%~5%。
   应用场景：形状识别（三角形、矩形、圆形...）、轮廓简化。

4. 用轮廓做形状识别的思路
   1) 图像预处理：灰度化 → 滤波 → 二值化
   2) 查找轮廓
   3) 筛选出面积合适的轮廓（过滤噪点）
   4) 对每个轮廓做多边形近似（approxPolyDP）
   5) 根据近似多边形的顶点数判断形状（3个=三角形，4个=四边形...）

5. 霍夫变换的直觉理解
   图像空间的一条直线，对应参数空间（rho-theta空间）的一个点。
   图像空间中一条直线上的所有点，在参数空间中都会经过同一个点。
   所以在参数空间中找"投票"最多的点，就对应图像空间中的直线。
   很巧妙的思路，就是参数空间离散化的精度需要权衡。

6. HoughLines vs HoughLinesP
   - HoughLines：标准霍夫变换，返回 (rho, theta)，是整条直线
   - HoughLinesP：概率霍夫变换，返回线段的两个端点 (x1, y1, x2, y2)
   实际用的时候 HoughLinesP 更多，因为返回的是线段端点，更方便处理，
   而且速度更快。
============================================
"""
