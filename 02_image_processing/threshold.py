...
1. 全局阈值 vs 自适应阈值
   - 全局阈值：整幅图用同一个阈值，适合光照均匀、目标和背景差异明显的图
   - 自适应阈值：每个像素根据周围邻域计算局部阈值，适合光照不均匀的图
   实际场景中光照不均匀很常见，所以自适应阈值用得很多。

2. Otsu 大津法的适用条件
   Otsu 法自动计算最优阈值的前提是图像的直方图有双峰（前景和背景两个峰）。
   如果直方图是单峰的，Otsu 算出来的阈值可能不对。

3. 形态学操作真的很实用
   开运算（先腐蚀后膨胀）：去除小的白色噪点，断开细连接
   闭运算（先膨胀后腐蚀）：填充小的黑色空洞，连接邻近区域
   这两个操作是二值图后处理的标配，几乎每次做完阈值分割都要来一下。

4. 结构元素的选择
   - 矩形核（MORPH_RECT）：最常用，各向同性
   - 椭圆核（MORPH_ELLIPSE）：圆形结构，适合处理圆形物体
   - 十字核（MORPH_CROSS）：各向异性，有方向性
   核越大，效果越强，但也越容易改变原物体的形状。

5. 我的二值图处理流程
   灰度图 → 高斯模糊去噪 → 阈值分割（全局/自适应/Otsu） → 形态学操作
   （开运算去噪点 → 闭运算填空洞）
   这套流程是通用的，大部分场景都能用。
============================================
"""
